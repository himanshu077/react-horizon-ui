    ...
    <div className="h-screen !bg-[--signin-bg] !min-w-[290px] m-0 py-8 !pl-8 sticky top-0 left-0 overflow-auto">
      <div className="!mb-24">
        <h3 className="Poppins700 !text-[26px] text-[--logo-color]">
          HORIZON{" "}
          <span className="Poppins400 text-[26px] text-[--logo-color]">
            FREE
          </span>
        </h3>
      </div>
        ...
        <ul>
          {data.map((list) => (
            <Link to={list.path} key={list.id}>
              <li
                className={`flex flex-row gap-8 py-3 ${
                  location.pathname === list.path
                    ? "text-[--logo-color] !border-r-4 !border-r-[--divider-color]"
                    : "text-[--tab-color]"
                }`}
              >
                <img src={list.src} alt={list.alt} />
                <p className="text-base leading-[30px] Poppins500">
                  {list.primary}
                </p>
              </li>
            </Link>
